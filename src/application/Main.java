...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			Button btnTop = new Button("Add new Account");
			Button btnMid = new Button("Add new Transaction");
			Button btnBottom = new Button("Schedule Transaction");
			root.add(btnTop, 1, 2);
			root.add(btnMid, 1, 3);
			root.add(btnBottom, 1, 4);
			
			//When you click "Add new Account" it runs this so that the program switches scene the the new account page
			btnTop.setOnAction(e -> primaryStage.setScene(addNewAccountScene)); 
			...
			newAcc.getChildren().addAll(addNewAccountText, accountNameInput, accountOpeningDateInput, accountOpeningBalance, confirmBtn, cancelBtn); 
			
			//if the user presses confirm without entering all the fields, then it prompts the user to enter all the fields
			confirmBtn.setOnAction(e -> {
				if(accountNameInput.getText().isEmpty() || accountOpeningDateInput.getText().isEmpty() || accountOpeningBalance.getText().isEmpty()) {
